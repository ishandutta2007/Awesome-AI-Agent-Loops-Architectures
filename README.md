# AI Agent Loops & Architecture Patterns

An **AI Agent Loop** is an iterative architectural pattern where a Large Language Model (LLM) continuously reasons, executes actions through external tools, observes the outcomes, and repeats the cycle until it fulfills a designated goal. Instead of relying on a human to manually provide the next prompt, the system autonomously passes its own tool outputs back into the context window as the next input.
...
3. **Observe**: The application executes the tool and appends the raw result back into the model's chat history.
...
def run_agent_loop(user_prompt: str, max_iterations=5):
    ...
    for iteration in range(max_iterations):
        ...
        # 2. ACT: Extract tool and call arguments
        if "ACTION:" in llm_response:
            ...
            # 3. OBSERVE: Fetch tool result and append back to the environment state
...
When working with standard ecosystems, high-level frameworks like [LangGraph](https://github.com) handle message persistence, exceptions, and graph cycles out of the box.
...
## Loop Controls & Production Concerns
